...
import { MdAdd, MdEdit, MdDelete, MdClose, MdSave, MdSelfImprovement, MdCheckCircle, MdStar } from "react-icons/md";
import { api } from "../../../api";
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="bg-purple-500 px-7 py-5 flex justify-between items-center">
          <h2 className="text-2xl font-extrabold text-white flex items-center gap-2">
            {entry ? <><MdEdit /> Edit Life Skill</> : <><MdAdd /> Add Life Skill</>}
          </h2>
        ...
        <div className="px-7 py-6 space-y-5 max-h-[65vh] overflow-y-auto">
          <div>
            <label className="block text-base font-bold text-gray-700 mb-2">Skill Name <span className="text-red-500">*</span></label>
            ...
            <div className="flex gap-3">
              {LEVELS.map(l => (
                <button key={l} type="button" onClick={() => toggleLevel(l)}
                  className={`flex-1 py-3 rounded-xl text-base font-bold border-2 transition ${form.levels.includes(l) ? "border-purple-500 bg-purple-50 text-purple-600" : "border-gray-200 text-gray-400"}`}>
                  {l}
                </button>
              ))}
            </div>
        ...
        <div className="px-7 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
          ...
          <button onClick={() => valid && onSave(form)} disabled={!valid || saving}
            className={`px-8 py-3 rounded-xl text-white text-base font-bold transition shadow flex items-center gap-2 ${valid && !saving ? "bg-purple-500 hover:bg-purple-600" : "bg-gray-300 cursor-not-allowed"}`}>
            <MdSave /> {saving ? "Saving…" : entry ? "Update" : "Add Life Skill"}
          </button>
        </div>
      </div>
    </div>
...
const LifeSkill = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Delete this life skill?")) return;
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center shadow-lg shrink-0">
            <MdSelfImprovement className="text-white text-4xl" />
          </div>
          <div>
            <h1 className="text-5xl font-extrabold text-gray-800">Life Skills</h1>
            <p className="text-gray-500 text-base mt-1">Manage life skill topics — {items.length} skills</p>
          </div>
        </div>
        <button onClick={() => { setEditItem(null); setModalOpen(true); }}
          className="flex items-center gap-2 bg-purple-500 text-white px-7 py-4 rounded-xl font-bold text-base shadow-lg hover:bg-purple-600 transition">
          <MdAdd className="text-2xl" /> Add Life Skill
        </button>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-gradient-to-r from-purple-500 to-purple-600 text-white">
              <th className="px-6 py-5 font-bold uppercase tracking-wider text-base">Skill</th>
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={4} className="text-center py-24 text-gray-400">
                <MdSelfImprovement className="text-7xl text-gray-300 mx-auto mb-4" />
                <p className="text-lg font-medium">No life skills yet.</p>
              </td></tr>
            ) : items.map((item, i) => (
              <tr key={item._id} className={`border-t border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-slate-50"} hover:bg-purple-50/30 transition-colors`}>
